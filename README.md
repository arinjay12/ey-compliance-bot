...
| 5 | Audio input (ask by voice) + spoken answers | ✅ Complete |
...
### Level 5 — Voice In and Out
- **Ask by voice:** a built-in microphone records your question, transcribed
  **locally** with faster-whisper (CPU, offline). The text flows into the same Q&A
  pipeline as typed questions. Model downloads once (~140 MB) on first use.
- **Spoken answers:** a "🔊 Listen" button under each answer reads it aloud using
  the offline pyttsx3 voice (Windows SAPI5). Citation tags are stripped first so it
  sounds natural.
- Both directions run fully offline — nothing leaves the machine, consistent with
